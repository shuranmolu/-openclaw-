...
async function handleCardActionEvent(ctx, data) {
    try {
        ...
        if (askResult !== undefined)
            return askResult;
        const memoryResult = await handleMemoryDecisionCardAction(ctx, data);
...
async function handleMemoryDecisionCardAction(ctx, data) {
    const event = data;
    const value = event?.action?.value;
    const action = value && typeof value === 'object' ? value.action : undefined;
    const memoryActions = new Set(['confirm_decision', 'update_decision', 'ignore_decision', 'mark_conflict']);
    if (!action || !memoryActions.has(action))
        return undefined;
    ...
    if (!chatId || !senderOpenId) {
        return { toast: { type: 'warning', content: '无法识别当前会话或操作者，请在群里用文字命令处理该决策卡。' } };
    }
    const payload = {
        action,
    ...
    const text = [
        '用户点击了决策卡治理按钮。',
        '请调用 feishu_memory_governance_action 处理该动作；如果需要同步多维表格，再调用 feishu_memory_bitable_sync 或官方 Bitable record update/create 工具。',
    ...
    await (0, synthetic_message_1.dispatchSyntheticTextMessage)({
        ...
        senderOpenId,
        text,
        syntheticMessageId: `memory-card-action:${Date.now()}`,
        chatType: event.context?.chat_type ?? 'group',
        runtime: ctx.runtime,
        forceMention: true,
    });
    return { toast: { type: 'success', content: '已收到操作，OpenClaw 会继续处理决策卡。' } };
}
